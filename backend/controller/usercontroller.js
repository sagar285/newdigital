...
exports.getuserlevelone = async (req, res) => {
  try {
    ...
    // console.log(user);
    // console.log(user.level1.length,levels.length);
    let level1userarray = [];
    var a = 0;
    ...
    userleveloneincome(level1userarray.length, user._id);
    res.send({ user1arrya: level1userarray });
    console.log(level1userarray.length);
  } catch (error) {
...
exports.getuserleveltwo = async (req, res) => {
  try {
    ...
    userleveltwoincome(level2userarray.length, user._id);
...
exports.getuserlevelthree = async (req, res) => {
  try {
    ...
    userlevelthreeincome(level3userarray.length, user._id);
...
exports.adminupi=async(req,res)=>{
    ...
    if(adminverify.role===1){
        ...
        if(alreadyupiid){
            const updateupiid= await Admincredential.findOneAndUpdate({admin:req.user._id},{upiid:upi},{new:true});
            return res.status(200).send(updateupiid);
        }
        else{
            const newupiid = await Admincredential({upiid:upi,admin:req.user._id});
...
exports.getadminupi=async(req,res)=>{
    try {
    const adminupi =await Admincredential.findOne({admin:req.user._id}).select("upiid");
    if(adminupi){
...
exports.getadminqrcode=async(req,res)=>{
    try {
    const adminupi =await Admincredential.findOne({admin:req.user._id}).select("QRcode");
    if(adminupi){
...
exports.adminqrcode =async(req,res)=>{
    ...
    if(adminverify.role===1){
        ...
        const alreadyupiid =await Admincredential.findOne({admin:req.user._id})
        if(alreadyupiid){
            const updateqrcode= await Admincredential.findOneAndUpdate({admin:req.user._id},
                { QRcode:{
...
exports.withdrawhistory = async (req, res) => {
  ...
  const withdrawrequestuser =await Withdrawlhistory.findOne({user:req.user._id,status:0});
  if(withdrawrequestuser){
        return res.status(201).send({message:"your previousrequest in pending, first this will confirmed"});
  }
 else{
    if(walletamount){
...
exports.rechargerequest = async (req, res) => {
  try {
    ...
    const alreadyrecharge = await Rechargemodel.findOne({ user: req.user._id ,status:1});
    if (alreadyrecharge) {
